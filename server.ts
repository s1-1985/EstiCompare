...
3. **旧単価賃率設定**: KNOWLEDGEの設備相場（セクション6の表）を基準とした標準賃率を設定する。目標売価との辻褄は主にSGA率(5〜${maxSgaOld}%以内・推奨10〜18%)の調整で吸収すること。SGA率の範囲に収まらない場合のみ賃率を比例調整するが、KNOWLEDGE相場の上限値の2倍を超えないこと。
4. **新単価賃率設定**: 旧単価と同じKNOWLEDGE相場を基準とし、旧単価よりやや高い賃率を設定。目標売価との辻褄はSGA率(5〜${maxSgaNew}%以内・推奨10〜18%)で吸収することを優先する。
