...
## 第9セッション（2026-05-29）で実施した変更（PR #65）

### PR #65: ラストワンマイル4件修正

1. **Excel出力に変動理由（changeReason）を追加**
   - 材料セクション・加工工程セクションの両方に `変動理由` 列を追加
   - 列幅 30文字分を確保

2. **XLSX.jsを動的インポートに変更**
   - `import * as XLSX from 'xlsx'` → `const XLSX = await import('xlsx')`
   - 初期バンドルから約1MBのXLSX.jsを除外し、Excelボタン押下時のみロード

3. **AI自動補正のバグ修正**（`handleAiAutoReconcile` in `App.tsx`）
   - `apiPost()` の戻り値（`Response`）に `.json()` を呼ぶのを忘れていた
   - AI自動補正モーダルの工程賃率が常に `undefined` になっていたのを修正

4. **保存成功通知をフローティングトーストに変更**
...
## 第11セッション（2026-06-01〜06-08）で実施した変更（PR #69・#71〜#76）

新旧比較の独立機能化〜実態加工費まわりの一連の改善。PR #69・#71〜#75 はマージ・本番デプロイ済み。**PR #76 のみ open（マージ可能・ユーザー手動マージ待ち）**。

### マージ済み
- **PR #69**: 複数Lot見積・複数品番比較を新旧比較から独立した機能に整理。シナリオに作成機能種別 `kind` を保存し読込時に正しい機能で開く。マイシナリオを機能別タブ化。複数品番に架空利管費率（積上）行。賃率100円端数警告など交渉防衛強化。
- **PR #71**: 工程ロック（ロック中はAI自動設定・自動補正の対象外）／「工程を追加」で上限撤廃・デフォルト5工程／送料/個の直接入力／目標単価ロックUI（開錠・施錠トグル）／Sheet3見積書の刷新（右端見切れ修正・賃率→分単価表記等）／シナリオ保存失敗（Firestore undefined）と利管費率の修正。
- **PR #72**: 各工程の「実態賃率(円/h)」入力を廃止し **「実態加工費(円/個)」直接入力に置換**（`ProcessRow.actualProcessingCostPerUnit`、>0なら実態原価として直接使用）。「実態加工費AI」ボタンで、調達先提示の内訳不明な加工費から賃率・出来高・段取を相場逆算（削り代・製品単価・ロット考慮）。`server.ts` に `/api/infer-breakdown-from-cost` 追加。
- **PR #73**: 見積書(PrintSheet)の旧/新境界の切り取り線を削除／スクラップ0は見積書に非表示／加工内訳の出来高下に加工タクト（`秒/個 = 3600÷出来高`）を表示。
- **PR #74**: Excel/CSV出力の小数桁修正。`calculateEstimate` の戻り値を全て `r3`（小数3位四捨五入）、PrintSheetのExcel書込前に `r2`（小数2位）。
- **PR #75**: UI改善。必須フィールド未入力を赤背景+赤ボーダー+ringで強調・必須マーク`*`を太字赤・入力文字を`text-sm`・フォームボーダーを濃く(`#A09488`)・薄グレー文字を`#5C5248`へ等。

### open（**マージ可能・ユーザーが手動マージ予定**）
- **PR #76**: https://github.com/s1-1985/EstiCompare/pull/76 （head `425a7de`、base `main 4f089a9`、`mergeable_state: clean`）
  1. **上部KPI並び替え**（新旧両方）: `仕入実費 → 現行単価(目標単価) → 実態利益率 → 積み上げ単価 → 架空利管費率`。
  2. **古いツールチップ修正**（全件監査）: 客提示賃率（実態賃率→実態加工費）・仕入実費の説明明確化。
  3. **実態加工費AIの諸元尊重＋「新旧整合」ボタン**（Part3）:
     - 実態加工費AIで逆算する際、反対側（旧↔新）同名工程に出来高・段取があればアンカーとして尊重し賃率のみローカルで解く（`rate = cost / (1/出来高 + 段取/ロット)`、100円単位）。諸元の無い工程のみAIへ。
     - 「新旧整合」ボタン（紫・`ArrowLeftRight`）: 旧/新同名standard工程（未ロック）を双方の諸元から整合。`/api/reconcile-process-physicals`（新規・Gemini構造化出力）が合意出来高・段取を返し、現行の客提示加工費を保つよう各側賃率を再計算。

### 追加されたAPIエンドポイント（server.ts）
- `POST /api/infer-breakdown-from-cost`（PR #72）— 実態加工費から賃率・出来高・段取を逆算
- `POST /api/reconcile-process-physicals`（PR #76）— 旧/新の出来高・段取を整合
- `POST /api/analyze-scenario` — シナリオのパターン分析（Firestoreへ永続化）

### ⚠️ GitHubアカウント停止インシデント（記録）
- セッション中、自動化されたgit push/PR操作が続いたことで **`s1-1985` アカウントが一時停止**（git/MCPともに403）。
- 復旧手順: support.github.com の「Account restrictions」フォームから**サインインせず**問い合わせ（停止解除）。**アカウント登録メール（Hotmail）から返信する必要あり**（別メールでは本人確認不可）。GitHubサポートのメールは `github.com` または `githubsupport.com` から届く（後者も公式）。
