...
| #49 | UIクリーンアップ：ウォーターフォール削除・AI疎通ボタン削除・行ズレ修正・ラベル改名・ロットデフォルト修正 |
...
## 第5セッション（2026-05-27 深夜）で実施した変更（PR #49）

大規模UIクリーンアップ。ユーザーが「いらない・わかりにくい」と指摘した要素を一括整理。

### 実装内容

1. **ウォーターフォールチャート削除** — `ExcelGrid.tsx`
   - `renderDiffBar` を `() => null` にスタブ化 → 旧コードを完全削除

2. **AI疎通確認フローティングボタン削除** — `App.tsx`
   - `aiTestStatus` / `aiTestMsg` state削除
   - `AlertTriangle` import削除

3. **KPI行の高さ固定（行ズレ解消）** — `App.tsx`
   - 条件付きレンダリングで行が増減していた → 常時表示（null時は `—`）に変更
   - 左右パネルの行位置が一致するようになった

4. **㉓/㉔/㉙/㉚ 外掛け・内掛け表示削除** — `App.tsx`
   - 固定ヘッダーの旧/新パネルから外掛け・内掛け利益率の表示行を削除
   - ユーザー「何の意味があんの？」→ 判断の余地なし

5. **利管費固定額・客向内掛け率フィールド削除** — `App.tsx`
   - Section 5 Row 3 を両パネルから削除

6. **材工費ベース客向け内掛け表示削除** — `App.tsx`
   - 削除理由: 送料・その他調整を含まない計算なので、この率を利管費率に使うと売値が実際の目標を超える誤解を生む
   - ユーザー実証「13.65%を利管費率に入力すると売値が100円超えた」

7. **帳尻利管費率の位置修正** — `App.tsx`
   - 新パネルで「利管費率入力の下」に出ていたのを「上」に移動（旧パネルと統一）

8. **計算仕入値 → 積み上げ単価** への改名 — `App.tsx`
   - `grandTotalUnitPrice` = 材料費+加工費+利管費+送料+調整 なので「仕入値」は誤解を招く
   - 全箇所を `replace_all` で一括置換

9. **`baseLotSize` デフォルト: 1 → 100** — `src/data/samples.ts`
   - 旧デフォルト `1` だと段取り1h/ロットが1個に全量配賦されて加工費 ¥1016.67（出来高60/段取り1/賃率¥1000）
   - 計算式: `(1/60 + 1/1) × 1000 = ¥1016.67`
   - 修正後: `(1/60 + 1/100) × 1000 = ¥26.67`
